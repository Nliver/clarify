---
title: '整体架构'
description: 'Clarify 采用 Monorepo 结构组织，包含应用和包两个层级。'
...
| 类型 | 所属 | 用途 |
|------|------|------|
| `ResolvedProjectConfig` | CLI | 已解析的站点元信息、部署前缀、品牌、主题、导航、多语言、变量和功能配置。 |
| `ResolvedBuildOptions` | CLI | 项目根目录、内容根目录和输出目录。 |
...
Clarify 内置 OpenAPI、content artifacts、HTML shell 等能力都通过插件 hook 接入。Hook 分为生命周期（`before:`/`after:` 对）、内容/路由管线、构建产物和拦截四类。当前实现的 hook 包括：
...
| `build:shouldRun` / `ssg:shouldRun` | 拦截构建或 SSG 阶段。 |
