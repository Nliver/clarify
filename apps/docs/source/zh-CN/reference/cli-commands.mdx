---
title: 'CLI 命令参考'
description: '@clarify-labs/cli 提供 clarify 命令，封装开发服务器、静态构建和项目初始化。用户不需要创建 Vite 配置文件。'
keywords: ['clarify', 'documentation', 'reference', 'cli-commands']
---

# CLI 命令参考

`@clarify-labs/cli` 提供 `clarify` 命令，封装开发服务器、静态构建和项目初始化。用户不需要创建 Vite 配置文件。

---

## 安装

对于新项目，最短路径是直接通过已发布包完成初始化：

<CodeGroup>

```bash title="pnpm"
pnpm dlx @clarify-labs/cli init my-docs
cd my-docs
pnpm dev
```

```bash title="npm"
npx @clarify-labs/cli init my-docs
cd my-docs
npm run dev
```

```bash title="yarn"
yarn dlx @clarify-labs/cli init my-docs
cd my-docs
yarn dev
```

</CodeGroup>

如果你是把 Clarify 接入现有项目，推荐把 CLI 安装为项目开发依赖，这样 CLI 版本会跟随仓库一起被锁定和 Review：

<CodeGroup>

```bash title="pnpm"
pnpm add -D @clarify-labs/cli
```

```bash title="npm"
npm install -D @clarify-labs/cli
```

```bash title="yarn"
yarn add -D @clarify-labs/cli
```

</CodeGroup>

然后可以通过 `pnpm exec` 直接调用：

```bash
pnpm exec clarify dev
pnpm exec clarify check
pnpm exec clarify build
```

或者在 `package.json` 中添加脚本，并优先通过包管理器脚本入口启动：

```json title="package.json"
{
  "scripts": {
    "dev": "clarify dev",
    "build": "clarify build"
  }
}
```

如果只是临时体验，也可以全局安装后直接运行 `clarify dev` / `clarify build`，但团队项目建议优先使用项目本地依赖。

---

## `clarify init`

创建一个 Clarify 项目骨架，并自动更新 `package.json` 中的 `dev` 和 `build` 脚本。

```bash
npx @clarify-labs/cli init my-docs
```

默认情况下，`clarify init` 会生成或更新以下内容：

- `clarify.ts`
- `source/index.mdx`
- `package.json` 中的 `dev` / `build` 脚本
- `@clarify-labs/cli` 开发依赖

如果希望初始化完成后立即安装依赖，可以使用：

```bash
npx @clarify-labs/cli init my-docs --install
```

未使用 `--install` 时，命令会提示下一步操作，例如：

```bash
cd <project-root>
npm install
npm run dev
```

常用选项：

| 选项 | 说明 | 默认值 |
|------|------|--------|
| `[directory]` | 目标项目目录 | 当前目录 |
| `--content <dir>` | 内容目录 | `source` |
| `--template <name>` | `minimal`、`standard` 或 `complete` 模板 | `standard` |
| `--force` | 覆盖已存在的初始化文件 | `false` |
| `--install` | 初始化后安装依赖 | `false` |

模板适用场景：

| 模板 | 适合场景 |
|------|----------|
| `minimal` | 只想快速验证 Clarify 或接入已有内容目录 |
| `standard` | 新建一个常规文档站，包含推荐的基础结构 |
| `complete` | 学习完整能力，查看导航、组件、OpenAPI 和发布配置示例 |

---

## `clarify dev`

启动本地文档开发服务器。

```bash
npm run dev
```

这是更推荐的用户入口，因为无论 `clarify` 来自项目本地依赖还是全局安装，团队使用方式都一致。底层脚本通常仍然是 `clarify dev`。

<CodeGroup>

```bash title="pnpm"
pnpm dev
```

```bash title="npm"
npm run dev
```

```bash title="yarn"
yarn dev
```

</CodeGroup>

常用选项：

| 选项 | 说明 | 默认值 |
|------|------|--------|
| `--root <dir>` | 项目根目录 | 当前目录 |
| `--content <dir>` | 内容目录 | `source` |
| `--host [host]` | 监听地址 | Vite 默认值 |
| `--port <port>` | 端口号 | Vite 默认值 |
| `--open [path]` | 启动后打开浏览器 | `false` |

示例：

```bash
pnpm exec clarify dev --port 5173 --open
```

---

## `clarify build`

构建可部署的静态站点。

```bash
pnpm exec clarify build
```

常用选项：

| 选项 | 说明 | 默认值 |
|------|------|--------|
| `--root <dir>` | 项目根目录 | 当前目录 |
| `--content <dir>` | 内容目录 | `source` |
| `--output <dir>` | 输出目录 | `output` |

示例：

```bash
pnpm exec clarify build --output dist
```

配置了 `siteUrl` 后，构建会额外输出 `sitemap.xml` 和 `robots.txt`。

---

## `clarify check`

在不产出构建文件的情况下检查文档项目。

```bash
pnpm exec clarify check
```

`check` 会验证内容目录、检测重复路由、标记 i18n fallback 路由，并检查本地 Markdown 链接。它适合在 CI 或发布前做一次快速质量门禁。

常用选项：

| 选项 | 说明 | 默认值 |
|------|------|--------|
| `--root <dir>` | 项目根目录 | 当前目录 |
| `--content <dir>` | 内容目录 | `source` |
| `--strict` | 将 warning 也视为失败 | `false` |
| `--format <format>` | `text` 或 `json` | `text` |

---

## `clarify mcp`

启动本地 stdio MCP server，为一个或多个已部署的 Clarify 站点提供 `search_docs` 工具。

```bash
npx @clarify-labs/cli mcp https://docs.example.com
```

该命令会获取 `<site>/mcp.json`，下载清单中声明的搜索索引，并缓存到 `~/.clarify/mcp-cache`。传入多个 URL，可以通过一个 MCP server 搜索多个文档站点：

```bash
npx @clarify-labs/cli mcp https://product.example.com https://api.example.com
```

| 参数或选项 | 说明 | 默认值 |
| --- | --- | --- |
| `<site> [site...]` | 已部署 Clarify 站点的根 URL | 必填 |
| `--no-cache` | 跳过本地缓存，每次获取最新索引 | `false` |

该命令将 stdout 用作 MCP JSON-RPC 通道，诊断信息写入 stderr。通常应由 MCP 客户端启动它，而不是把它作为持续运行的交互式终端命令。客户端配置、工具参数、构建产物和故障排查参见 [MCP 文档搜索](/features/mcp)。

---

## 配置文件

推荐使用 `clarify.ts` 作为唯一项目配置入口。它既可以配置站点展示字段，也可以配置构建期扩展能力：

```ts
import { defineConfig } from '@clarify-labs/cli'

export default defineConfig({
  title: 'Clarify Docs',
  description: 'Documentation powered by Clarify',
  theme: { preset: 'default' },
  navigation: {
    tabs: [
      {
        tab: 'Docs',
        icon: 'BookOpen',
        pages: 'FileTree',
      },
    ],
  },
  plugins: [
    // searchPlugin(),
    // translatePlugin(),
  ],
})
```

Clarify 会按以下优先级查找配置文件：

```
clarify.ts > clarify.js > clarify.json
```

`clarify.json` 适合不需要插件函数的简单项目；需要插件或更强类型提示时使用 `clarify.ts`。

---

## 配置优先级

CLI 选项用于控制运行时路径和输出目录，配置文件用于控制站点展示和构建期扩展。优先级为：

```
CLI 选项 > clarify.ts/js/json > 默认值
```

---

## 帮助与版本

```bash
clarify --help
clarify --version
```
