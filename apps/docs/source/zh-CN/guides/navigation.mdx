---
title: '配置站点'
description: '这篇文档合并说明 Clarify 中和“站点体验”有关的配置：顶部导航、Tabs、侧边栏、主题、多语言、部署子路径和信息架构。'
...
export default defineConfig({
  navigation: {
    menus: [
      { label: '开始使用', page: 'getting-started' },
      {
        label: '资源',
        items: [
          { label: 'API 文档', page: 'features/openapi' },
...
| `items` | 嵌套菜单项；每一项可继续使用 `href`、`page` 或 `items` |
| `description` | 展开菜单中可选的辅助说明 |
| `icon` | 可选的 Lucide 图标名 |
...
Clarify 使用 `navigation.tabs` 做站点的一层业务分类。每个 tab 拥有自己的侧边栏。
