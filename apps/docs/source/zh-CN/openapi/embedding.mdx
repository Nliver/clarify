---
title: 'OpenAPI 嵌入使用说明'
description: 'Clarify 不只会把 .openapi.json / .openapi.yaml 渲染成完整 API Reference，也允许你在 MDX 指南里嵌入单个接口或完整 Reference。这样可以把“接口事实”和“业务流程说明”放在同一个页面里。'
...
| 让读者不离开指南就能发起请求并查看响应 | `OpenApiRequest` |
...
<FileTree aria-label="语言目录下的 OpenAPI 规范">
...
| `operationId` | 是 | 唯一的 OpenAPI `operationId`，例如 `demoBasicRequest` |

渲染效果：

<OpenApiOperation specPath="/api" operationId="demoBasicRequest" />

接口栏中的播放按钮会打开交互式请求测试器。如果发起请求本身就是指南流程的一部分，可以直接嵌入同一个测试器：

```mdx
<OpenApiRequest specPath="/api" operationId="demoBasicRequest" requestExample="published" />
```

`OpenApiRequest` 的基础参数与 `OpenApiOperation` 一致。通过 `requestExample` 指定 OpenAPI 请求体或参数 `examples` 中的 key；同名的参数和请求体示例会一起应用。它只渲染请求工作台，适合在教程中引导读者实际发起一次请求并查看响应。
...
- 在关键步骤旁嵌入 `OpenApiOperation`，减少读者在指南和 Reference 之间来回跳转。
