---
title: 'Mermaid'
description: 'Mermaid renders diagrams from text source. Use it for flows, sequences, state transitions, and relationships that should stay reviewable in version control.'
keywords: ['clarify', 'documentation', 'components', 'mermaid']
---

# Mermaid

`Mermaid` renders diagrams from text source. Use it for flows, sequences, state transitions, and relationships that should stay reviewable in version control.

## Usage

Use a `mermaid` code fence for the shortest authoring experience:

````md
```mermaid
flowchart LR
  Write[Write MDX] --> Build[Build site]
  Build --> Publish[Publish docs]
```
````

Rendered result:

```mermaid
flowchart LR
  Write[Write MDX] --> Build[Build site]
  Build --> Publish[Publish docs]
```

In generated MDX, call the component directly with `chart`:

```mdx
<Mermaid chart={'sequenceDiagram\n  Author->>Clarify: Save content\n  Clarify-->>Reader: Updated page'} />
```

## Props

<Properties>
  <Property name="chart" type="string">
    Required. Mermaid diagram source.
  </Property>
  <Property name="className" type="string">
    Optional class name for the diagram container.
  </Property>
</Properties>

<Callout type="tip">Prefer labels that still explain the flow when the raw source is shown during loading or when rendering fails.</Callout>
