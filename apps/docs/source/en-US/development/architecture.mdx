...
description: 'Clarify uses a Monorepo structure with two levels: apps and packages.'
keywords: ['clarify', 'documentation', 'development', 'architecture']
---
...
| Type | Owner | Purpose |
|------|-------|---------|
| `ResolvedProjectConfig` | CLI | Resolved site metadata, deployment prefixes, branding, theme, navigation, locales, variables, and features. |
| `ResolvedBuildOptions` | CLI | Project root, content root directory, and output directory. |
...
Clarify's built-in capabilities such as OpenAPI, content artifacts, and the HTML shell are all connected through plugin hooks. Hooks are grouped into lifecycle (`before:`/`after:` pairs), content/routing pipeline, build artifacts, and intercept categories. The currently implemented hooks include:
...
| `content:transform` | Rewrite or enrich content before it becomes a page. |
| `pages:resolved` | Inspect or reshape the page list. |
...
| `build:shouldRun` / `ssg:shouldRun` | Intercept the build or SSG stage. |
